        ...
        <div class="flex flex-wrap justify-center gap-3 mb-12">
            ...
            @foreach($categories as $cat)
                <button wire:click="filterByCategory('{{ $cat->slug }}')" 
                        class="px-5 py-2 rounded-full text-sm font-medium transition {{ $category === $cat->slug ? 'bg-kamina-blue text-white shadow-md' : 'bg-white text-gray-600 hover:bg-gray-100' }}">
                    {{ $cat->name }}
                </button>
            @endforeach
        </div>

        <!-- Grille Articles (Réutilisation du style Home avec pagination) -->
