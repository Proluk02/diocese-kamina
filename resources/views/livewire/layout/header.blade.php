...
<!-- Z-INDEX RÉDUIT À 30 pour rester sous la Modale (999) et la Sidebar Mobile -->
    ...
    <div class="flex flex-grow items-center justify-between py-3 px-4 md:px-6">
        ...
        <div class="flex items-center gap-3">
            ...
            <button @click="$store.theme.toggle()" 
                    class="p-2.5 rounded-full text-gray-500 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-800 dark:hover:text-yellow-400 hover:text-orange-500 transition-all duration-300">
            ...
            <div x-data="{ dropdownOpen: false }" class="relative">
                ...
                <div x-show="dropdownOpen" x-cloak 
                     ...
                     <div class="flex justify-between items-center px-5 py-4 border-b border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/50 backdrop-blur-sm">
                        ...
                        @if($unreadNotificationsCount > 0)
                            <button wire:click="markAllAsRead" class="text-xs px-2 py-1 rounded bg-kamina-blue/10 text-kamina-blue dark:bg-kamina-gold/10 dark:text-kamina-gold font-medium hover:bg-kamina-blue/20 transition-colors">
                                Tout marquer lu
                            </button>
                        @endif
                     </div>
                        ...
                        @forelse ($notifications as $notification)
                            ...
                            <a href="{{ $notification->data['url'] ?? '#' }}" class="flex gap-4 px-5 py-4 border-b border-gray-100 dark:border-gray-800 transition-colors {{ $bgClass }}">
                                <div class="h-10 w-10 flex-shrink-0 rounded-full flex items-center justify-center 
                                    {{ $icon === 'user-plus' ? 'bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400' : '' }}
                                    {{ $icon === 'document-text' ? 'bg-green-100 text-green-600 dark:bg-green-900/30 dark:text-green-400' : '' }}
                                    {{ $icon === 'default' ? 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400' : '' }}">
                                    
                                    @if($icon === 'user-plus') <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z"/></svg>
                                    @elseif($icon === 'document-text') <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
            ...
            <div x-data="{ dropdownOpen: false }" class="relative">
                <button @click="dropdownOpen = !dropdownOpen" 
                    ...
                    <div class="h-9 w-9 rounded-full bg-gradient-to-br from-kamina-gold to-yellow-600 text-white flex items-center justify-center font-bold shadow-md group-hover:scale-105 transition-transform">
                ...
                <div x-show="dropdownOpen" @click.outside="dropdownOpen = false" x-cloak
                     x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 translate-y-2"
                     class="absolute right-0 mt-3 w-60 rounded-2xl bg-white dark:bg-gray-900 shadow-2xl border border-gray-100 dark:border-gray-800 z-50 overflow-hidden ring-1 ring-black/5">
                    ...
                    <div class="py-2">
                        ...
                        <a href="#" class="flex items-center gap-3 px-5 py-2.5 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-kamina-blue dark:hover:text-kamina-gold transition-colors">
                    ...
                    <div class="border-t border-gray-100 dark:border-gray-800 py-2">
                        <button wire:click="logout" class="flex items-center gap-3 w-full text-left px-5 py-2.5 text-sm text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors">
